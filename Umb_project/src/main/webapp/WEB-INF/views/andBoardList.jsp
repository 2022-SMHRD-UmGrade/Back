...
function htmlView(data){
	...
	result += "<tr><th>일련번호</th><th>제목</th><th>내용</th><th>작성일시</th><th>파일경로</th><th>작성자</th><th>조회수</th></tr>"
	   ...
	   $.each(data, (index, vo)=>{ // 오브젝트안에 있는 데이터 접근.
		   result += "<tr>"
			      result += "<td>"+vo.article_seq+"</td>"
			      result += "<td>"+vo.article_title+"</td>"
			      result += "<td>"+vo.article_content+"</td>"
			      result += "<td>"+vo.article_file+"</td>"
			      result += "<td>"+vo.article_id+"</td>"
			      result += "<td>"+vo.article_cnt+"</td>"
			      result += "</tr>"
	   })
	   result += "</table>"
	   $("#list").html(result)
